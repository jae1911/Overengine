...
export const generateFeeds = (hostname: string): Feed => {
  ...
  const allThePosts: readonly PostMedatada[] = dirContent
    .map((entry: string): PostMedatada | undefined => {
  ...
  sortedPosts.forEach((postMeta) => {
    if (
      ...
      const formattedURi = `${protocol}://${hostname}/blog/${postMeta.pubDate.getFullYear()}/${String(
        postMeta.pubDate.getMonth() + 1,
      ).padStart(2, "0")}/${String(postMeta.pubDate.getUTCDate()).padStart(
        2,
        "0",
      )}/${postMeta.title
        .replaceAll(" ", "-")
