...
const bgpBaseUri = BGPAS ? `https://api.bgpview.io/asn/${BGPAS}/` : null;
...
export const getBgpUpstreams = async (): Promise<string | undefined> => {
  ...
  try {
    ...
    if (parsedJson.status != "ok") {
      ...
      const v4Upstreams: readonly string[] = parsedJson.data.ipv4_upstreams
        .map((upstream: bgpUpstream): string => {
          return `<li>${upstream.asn} - ${upstream.description}</li>`;
        })
        .filter((item) => item) as readonly string[];

      const v6Upstreams: readonly string[] = parsedJson.data.ipv6_upstreams
        .map((upstream: bgpUpstream): string => {
          return `<li>${upstream.asn} - ${upstream.description}</li>`;
        })
        .filter((item) => item) as readonly string[];
...
export const getWeatherForCity = async (
  ...
  const res = await axiosClient
    .get(`weather?q=${city ?? OWMCITY}&units=metric&appid=${OWMKEY}`)
    .catch((error) => {
      return {
        data: [
          {
  ...
  if (parsedJson.main == null) {
    ...
    const res = `Weather in ${city ?? OWMCITY} is ${
      parsedJson.weather[0].description
    } with ${parsedJson.main.temp}°C.`;
...
export const translateString = async (
  ...
  const jsonRes = JSON.parse(
    JSON.stringify(resFromLingva.data),
